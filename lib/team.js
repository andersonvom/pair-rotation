...
function byTimesAndRandomName(p1, p2) {
...
class Team {
  ...
  rotate(members = []) {
    ...
    const firstOccurrence = (pair) => {
      if (currentMembers[pair.name1] || currentMembers[pair.name2]) {
        return false
      }

      currentMembers[pair.name1] = true
      currentMembers[pair.name2] = true
      return true
    }

    return this.pairs()
      .sort(byTimesAndRandomName)
      .filter(bothMembers)
